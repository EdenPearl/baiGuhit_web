...
const CheckoutCart = ({ state, onClose, onOrderPlaced }) => {
  const [userId, setUserId] = useState(null);
  const cartItems = state?.cartItems || [];
  const totalPrice = state?.totalPrice || 0;
  const [showSuccess, setShowSuccess] = useState(false);
  const [errors, setErrors] = useState({ name: false, contact_number: false });
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!formData.name || !formData.contact_number || !formData.estimated_date) {
      setErrors({
        name: !formData.name,
    ...
    try {
      for (const item of cartItems) {
        ...
        // Explicitly consume the success response to prevent browser alert
        const data = await response.json();
        console.log(data); // Log the response for debugging (optional)
      }
  ...
  return (
    <>
      <ModalContainer>
        ...
        <CheckoutTitle>Checkout Cart</CheckoutTitle>
        <CheckoutInfo>
          <ProductSection>
            {cartItems.length > 0 ? (
              cartItems.map((item, index) => (
                <ProductDetails key={index}>
                  <ProductImage src={item.imageUrl} alt={item.product_name} />
                  <ProductInfo>
            ...
            <FormSection>
              <InfoText>
                Your order will be available for pickup on <strong>BISU-BILAR Campus</strong> at{' '}
                <strong>COMSOC office</strong> the day after purchase.
              </InfoText>
              <FormField>
                <Label>Name</Label>
                <Input
              ...
              <FormField>
                ...
                <Input
                  ...
                  required
                  style={{ borderColor: errors.contact_number ? "red" : "#ddd" }}
                />
              </FormField>
              <FormField>
              ...
              <FormField>
                ...
                <InfoText>Payment will be made in person upon pickup.</InfoText>
              </FormField>
            </FormSection>
          </ProductSection>
      ...
      {showSuccess && (
        <SuccessOverlay>
          <SuccessContainer>
            <SuccessImage
              src="https://via.placeholder.com/100?text=Success"
              alt="Order Success"
            />
            <InfoText>Order placed successfully!</InfoText>
            <ViewOrderButton onClick={handleViewOrder}>View Order</ViewOrderButton>
          </SuccessContainer>
        </SuccessOverlay>
      )}
    </>
